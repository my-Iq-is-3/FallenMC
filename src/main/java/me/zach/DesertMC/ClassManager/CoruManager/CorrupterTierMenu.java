...
public class CorrupterTierMenu {
	...
	public Inventory getInventory(boolean ghost){
		inventory = DesertMain.getInstance.getServer().createInventory(null, 27, ghost ? "Corrupter Class Rewards" : "Corrupter Class");
		for(int i = 0;i<27;i++) {
		...
		else nsml.add(ChatColor.YELLOW + "Click " + ChatColor.DARK_GRAY + "to select the " + ChatColor.YELLOW + "Corrupter " + ChatColor.DARK_GRAY + "Class!");
		nsm.setLore(nsml);
